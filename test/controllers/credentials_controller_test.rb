...
class CredentialsControllerTest < ActionController::TestCase
  ...
  test "should create credential" do
    assert_difference('Credential.count') do
      post :create, credential: { deploy: @credential.deploy, forecast: @credential.forecast, schema_id: @credential.schema_id, snapshot: @credential.snapshot, status: @credential.status, user_id: @credential.user_id }
    end
  ...
  test "should update credential" do
    patch :update, id: @credential, credential: { deploy: @credential.deploy, forecast: @credential.forecast, schema_id: @credential.schema_id, snapshot: @credential.snapshot, status: @credential.status, user_id: @credential.user_id }
    assert_redirected_to credential_path(assigns(:credential))
  end
