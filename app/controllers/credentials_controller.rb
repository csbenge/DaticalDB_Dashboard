class CredentialsController < ApplicationController
  ...
  def create
    ...
    credentials = Credential.where("schema_id = '#{@credential.schema_id}' AND user_id = '#{@credential.user_id}'")
    respond_to do |format|
      if credentials != nil 
        flash[:error] = 'Credential already exist for user/schema.'
        format.html { redirect_to @credential }
      elsif @credential.save
  ...
  def update
    respond_to do |format|
      if @credential.update(credential_params)
        format.html { redirect_to credentials_url, notice: 'Credential was successfully updated.' }
  ...
  private
    ...
    def credential_params
      params.require(:credential).permit(:user_id, :schema_id, :snapshot, :status, :forecast, :deploy)
    end
end
